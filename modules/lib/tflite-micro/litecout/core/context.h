...
class ContextScheduler {
 public:
  static ContextScheduler& Global() {
    cout<<"new ContextScheduler;"<<endl;
    static auto* x = new ContextScheduler;
    cout<<"new ContextScheduler after"<<endl;
    return *x;
  }
  ...
  std::unique_ptr<KernelContext> NewContext(
    ...
    cout<<"std::unique_ptr<KernelContext> "<<endl;
    std::unique_ptr<KernelContext> ctx(new KernelContext);
    cout<<"std::unique_ptr<KernelContext> ctx(new KernelContext);"<<endl;
    switch (target) {
      case TARGET(kHost):
 ...
 private:
  ...
  void InitContext() {
    cout<<"kernel_contexts_[Type].As<ContextT>().InitOnce();"<<endl;
    kernel_contexts_[Type].As<ContextT>().InitOnce();
    cout<<"kernel_contexts_[Type].As<ContextT>().InitOnce();  after "<<endl;
  }
...
#endif
   cout<<"InitContext<TargetType::kARM, ARMContext>();"<<endl;
   InitContext<TargetType::kARM, ARMContext>();
   cout<<"InitContext<TargetType::kARM, ARMContext>(); after"<<endl;
